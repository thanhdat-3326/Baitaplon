// em có chỉnh sửa một phần nhỏ trong phần code của em so với phần code trong báo cáo đã nộp đó là thêm chức năng sắp xếp sách cũng như người mượn và xóa trường số điện thoại trong cấu trúc người mượn
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
...
Sach* taoNodeSach(char* maSach, char* tenSach, char* tacGia, char* namXB, int soLuong, int giaSach) {
...
NguoiMuon* taoNodeNguoiMuon(char* mttv, char* hoTen, char* maSach, char* tenSach, int soLuong, char* ngayMuon, char* ngayTra) {
...
void themSach(Sach** head) {
    ...
    do {
        char maSach[10], tenSach[100], tacGia[50], namXB[15];
        int soLuong, giaSach;
        printf("Nhap ma sach: ");
        scanf("%s", maSach);
        getchar();
        ...
        scanf("%d", &giaSach);
        Sach* newNode = taoNodeSach(maSach, tenSach, tacGia, namXB, soLuong, giaSach);
        Sach* current = *head;
        Sach* prev = NULL;
...
void themNguoiMuon(NguoiMuon** head){
	...
	do {
	char hoTen[50], maSach[10], tenSach[100], ngayMuon[15], ngayTra[15];
    int soLuong;
    ...
    scanf("%s", ngayTra);
    getchar();
    NguoiMuon* newNode = taoNodeNguoiMuon(mttv, hoTen, maSach, tenSach, soLuong, ngayMuon, ngayTra);
...
void hienthiSach(Sach* head) {
    printf("\nDanh sach sach\n");
    printf("%-10s %-40s %-30s %-20s %-10s %-10s\n", "MS sach", "Ten sach", "Tac gia", "Nam xuat ban", "So luong", "Gia sach");
    while (head != NULL) {
        printf("%-10s %-40s %-30s %-20s %-10d %-10lld\n", head->maSach, head->tenSach, head->tacGia, head->namXB, head->soLuong, head->giaSach);
        head = head->next;
    }
}
...
void hienthiNguoiMuon(NguoiMuon* head) {
    ...
    printf("%-10s %-30s %-10s %-30s %-10s %-20s %-20s\n", "Ma the", "Ten nguoi muon", "MS sach", "Ten sach", "So Luong", "Ngay muon", "Ngay tra");
    while (head != NULL) {
        printf("%-10s %-30s %-10s %-30s %-10d %-20s %-20s\n", head->mttv, head->hoTen, head->maSach, head->tenSach, head->soLuong, head->ngayMuon, head->ngayTra);
        head = head->next;
    }
}
...
void ghifileSach(Sach* head, const char* filename) {
    ...
    while (head != NULL) {
        fprintf(f, "%-10s %-40s %-30s %-20s %-10d %-10lld\n", head->maSach, head->tenSach, head->tacGia, head->namXB, head->soLuong, head->giaSach);
        head = head->next;
    }
...
void ghifileNguoiMuon(NguoiMuon* head, const char* filename) {
    ...
    while (head != NULL) {
        fprintf(f, "%-10s %-30s %-10s %-30s %-10d %-20s %-20s\n", head->mttv, head->hoTen, head->maSach, head->tenSach, head->soLuong, head->ngayMuon, head->ngayTra);
        head = head->next;
    }
    ...
    for (NguoiMuon* i = *head; i != NULL; i = i->next) {
        for (NguoiMuon* j = i->next; j != NULL; j = j->next) {
            ...
            if ((tangDanMTTV && cmp > 0) || (!tangDanMTTV && cmp < 0)) {
                char tempMTTV[10], tempHoTen[100], tempMS[10], tempTenSach[100], tempNgayMuon[15], tempNgayTra[15];
                int tempSL;
    ...
    do {
        ...
        switch (choice) {
            ...
            case 3: {
                ...
                if (s) {
                	printf("%-10s %-40s %-30s %-20s %-10d %-10d\n", "MaSach", "TenSach", "TacGia", "NamXB", "SoLuong", "GiaSach");
                    printf("%-10s %-40s %-30s %-20s %-10d %-10d\n", s->maSach, s->tenSach, s->tacGia, s->namXB, s->soLuong, s->giaSach);
                } else {
            ...
            case 6: {
            	...
            	if (nm) {
            	    printf("%-10s %-30s %-10s %-30s %-10d %-20s %-20s\n", nm->mttv, nm->hoTen, nm->maSach, nm->tenSach, nm->soLuong, nm->ngayMuon, nm->ngayTra);
                } else {
